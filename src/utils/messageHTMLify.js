...
export const messageHTMLify = (
  ...
  while ((array1 = regexBoth.exec(message)) !== null) {
    //iterate the array for each match
    ...
    while ((array2 = regexSub.exec(array1[0])) !== null) {
      ...
      if (!colorIsBackground) {
        //if it is the first pair of numbers in the color code
    ...
    messageOutgoing += message.substring(
      previousLastIndex,
      regexBoth.lastIndex - array1[0].length
    ); //add in the first part of the message leading up the color code
