...
export const colors_light = [
  { colorName: "default", rgbValue: "38,38,38" }, //dark-grey
  { colorName: "Black", rgbValue: "0,0,0" }, //black
  { colorName: "Blue", rgbValue: "0,0,127" }, //blue
  { colorName: "Green", rgbValue: "0,147,0" }, //green
  { colorName: "LightRed", rgbValue: "255,0,0" }, //light red
...
export const colorNameToRGB = (name, lightTheme) => {
  let rgb;
  if (lightTheme) {
    rgb = colors_light.filter((color) => name === color.colorName)[0];
  } else {
    rgb = colors.filter((color) => name === color.colorName)[0];
  }
  if (rgb) {
    return rgb.rgbValue;
  } else {
